...
object Par {
    ...
    //  be the sole function in the API for controlling parallelism. We can always do `fork(map2(a,b)(f))` if we want the evaluation of `f` 
    ...
    //    --for one, the outer `Callable` will block waiting for the "inner" task to complete. Since this blocking occupies a thread 
    // in our thread pool, or whatever resource backs the `ExecutorService`, this implies that we're losing out on some potential parallelism. 
    // Essentially, we're using two threads when one should suffice. This is a symptom of a more serious problem with the implementation, 
...
object Examples {
    ...
    // List(1,2,3,4,5,6,7,8,9).toIndexedSeq
    //def main( args:Array[String]) : Unit = Console.println( Par.run( new ExecutorService())(Par.lazyUnit( sum(_))))
    def main( args:Array[String]) : Unit = {
        Console.println( List(1,2,3,4,5,6,7,8,9).toIndexedSeq)
        val a = Par.lazyUnit(42 + 1)
        val S = Executors.newFixedThreadPool(2) // with 1, it blocks
